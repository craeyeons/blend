...
def compute_l2_error_field(u_pred, v_pred, p_pred, u_true, v_true, p_true, layout):
    ...
    # Normalize pressure by its range
...
def compute_coverage_curve(pinn_pred, cfd_truth, router_output, layout, n_points=100):
    ...
    error_p = ((p_pinn - p_cfd) / p_range) ** 2
...
def compute_expected_losses(residual_field, router_output, layout, beta):
    ...
    # Hybrid with default threshold 0.5
    cfd_mask = confidences >= 0.5
...
def compute_loss_vs_coverage(residual_field, router_output, layout, beta, n_points=200,
    ...
    # Compute actual router loss
    mean_r = np.mean(confidences)
    actual_cfd_cost = beta * mean_r
    actual_residual_loss = np.mean((1 - confidences) * residuals)
    
    r_2d = router_output * layout
    tv_h = np.sum(np.abs(r_2d[:, 1:] - r_2d[:, :-1]))
    tv_v = np.sum(np.abs(r_2d[1:, :] - r_2d[:-1, :]))
    actual_tv_loss = lambda_tv * (tv_h + tv_v) / n_fluid
    
    conf_clipped = np.clip(confidences, 1e-7, 1 - 1e-7)
    actual_entropy = np.mean(-conf_clipped * np.log(conf_clipped) - 
                             (1 - conf_clipped) * np.log(1 - conf_clipped))
    actual_entropy_term = -lambda_entropy * actual_entropy
    
    actual_variance = np.var(confidences)
    actual_variance_term = -lambda_variance * actual_variance
    
    actual_total_loss = (actual_cfd_cost + actual_residual_loss + actual_tv_loss + 
                        actual_entropy_term + actual_variance_term)
    actual_coverage = mean_r
...
def plot_combined_metrics(coverage, accuracy, results, beta,
    ...
    ax1.plot(loss_coverage * 100, loss_values, 'b-', linewidth=2.5, label='Full Training Loss')
...
def plot_coverage_curve(coverage, accuracy, results, beta, save_path=None):
    """
    Plot the coverage curve (R² vs coverage).
    """
    fig, ax = plt.subplots(figsize=(10, 7))
    
    ax.plot(coverage * 100, accuracy, 'k-', linewidth=2.5, label='R² (PINN prediction quality)')
    
    ax.plot(0, accuracy[0], 'o', color='purple', markersize=12, zorder=5, 
            label=f'All PINN: R²={accuracy[0]:.4f}')
    ax.plot(100, accuracy[-1], 'o', color='teal', markersize=12, zorder=5, 
            label=f'All CFD: R²={accuracy[-1]:.4f}')
    ...
    ax.set_title('Coverage vs R² (PINN Prediction Quality) - Cavity Flow', fontsize=14)
